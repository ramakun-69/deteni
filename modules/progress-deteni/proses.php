...
// jika user sudah login, maka jalankan perintah untuk insert, update, dan delete
    ...
    if ($_GET['act'] == 'insert') {
        if (isset($_POST['simpan'])) {
            ...
            $progress        = mysqli_real_escape_string($mysqli, trim($_POST['progress']));


            $created_user    = $_SESSION['id_user'];

            $nama_file          = $kode_deteni . "-" . $_FILES['data']['name'];
            ...
            if (in_array($extension, $allowed_extensions)) {
                ...
                if ($ukuran_file <= 3000000) { // Cek apakah ukuran file yang diupload kurang dari sama dengan 1MB
                    ...
                    if (move_uploaded_file($tmp_file, $path_file)) { // Cek apakah gambar berhasil diupload atau tidak
                        ...
                        $query = mysqli_query($mysqli, "INSERT INTO is_obat_masuk(kode_transaksi,tanggal_masuk,kode_deteni,progress,created_user,data) 
                                            VALUES('$kode_transaksi','$tanggal_masuk','$kode_deteni','$progress','$created_user','$nama_file')")
                        ...
                        if ($query) {
                            ...
                            header("location: ../../main.php?module=progress_deteni&alert=1");
                        }
                    } else {
                        // Jika tipe file yang diupload bukan doc, docx, pdf, tampilkan pesan gagal upload
                        header("location: ../../main.php?module=progress_deteni&alert=2");
                    }
                } else {
                    // Jika tipe file yang diupload melebihi 3MB tampilkan pesan gagal upload
                    header("location: ../../main.php?module=progress_deteni&alert=3");
                }
            }
        }
    }
